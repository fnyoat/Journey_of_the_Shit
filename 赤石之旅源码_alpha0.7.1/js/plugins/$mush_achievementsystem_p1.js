...
MUSH_Achievements.prototype.createSceneDetails = function() {
	...
	this._snDetails = {};
		...
		//   - One line Max.
		//-----------------------------------------------
		this._snDetails.sceneDescription = "按Z切换视图";
		...
		//-----------------------------------------------
		this._snDetails.textCompletion = "完成:";
		...
		//-----------------------------------------------
		this._snDetails.textReward   = "奖励: ";
		this._snDetails.textNoReward = "";
...
MUSH_Achievements.prototype.createAchievementsList = function() {
	...
	//                  the next line.
	//
	//   - 奖励: Use the format: "type/index/amount".
	//     + type: 0 代表物品, 1 代表盔甲, 2 代表武器, 3 代表G. 设置为 -1 代表没有奖励.
	...
	//       	"0/1/3"    = 奖励物品#1 from the database 3 times.
	//       	"1/5/1"    = 奖励盔甲 #5 from the database once.
	//          "2/10/5"   = 奖励武器#10 from the database 5 times.	
	//          "3/0/1000" = 奖励1000G
	//          "-1/0/0"   = 没有奖励
	//
	...
	//     + type 1 (对于变量 ==) If type = 1, the condition for earning the achievement is set by a game
	...
	//     + type 2 （对于变量 >=） 如果 type = 2，则获得成就的条件由游戏设置
	//                        变量等于或大于某个值。Value1 确定游戏变量和
	//                        值 2 确定与之进行比较的值。
	//                        示例：“2/7/12” = if 变量 #3 >= 10。
	//     + type 3 (对于游戏时间) The condition here is game playtime in frames. If the playtime in frames is greater
	...
	//     + type 4 (对于步数) The condition here is the number of steps the player took. Value1 determines the number of
	...
	//-------------------------------------------------------------------------------
		...
		["史莱姆凝液",          67, "",         "这是泰拉瑞亚还是原神？", "-1/0/0",    "0/2/0", true, true, true] , 
		["跨世界交流",          4, "",         "你与主角的首次沟通", "-1/0/0",    "0/19/0", true, true, true] , 
		["CNM",          5, "",         "你被一只猫用恶毒的语言辱骂了", "-1/0/0",    "0/11/0", true, true, true] ,
		["起来重睡",          5, "",         "电影院骂骂咧咧的重新趴下睡觉", "-1/0/0",    "0/22/0", false, true, false] , 
		["赌博害人",          1, "",         "赌博输掉一次", "-1/0/0",    "0/4/0", true, true, true] , 
		["BAD APPLE",          265, "",         "吃掉坏苹果", "-1/0/0",    "0/6/0", true, true, true] , 
		["禁止触摸",          9, "",         "小心触电", "-1/0/0",    "0/16/0", true, true, true] , 
		["不该来的店",          84, "",         "未成年人不可以去哦", "-1/0/0",    "0/17/0", true, true, true] , 
		["赤石：恶臭神像",          89, "",         "激活赤石：恶臭神像", "-1/0/0",    "0/18/0", false, true, false] , 
		["原神，启动！",          244, "",         "启动原神", "-1/0/0",    "0/7/0", true, true, true] , 
		["你可以吃芝士汉堡",          274, "",         "喂电影院吃芝士汉堡", "-1/0/0",    "0/12/0", true, true, true] , 
		["孤注一掷",          17, "",         "赌上一切", "-1/0/0",    "0/14/0", true, true, true] , 
		["立起来的硬币",          88, "",         "[传奇成就]接受平局", "-1/0/0",    "0/15/0", true, true, true] , 
		["全成就收集",          87, "",         "解锁所有成就", "-1/0/0",    "0/13/0", false, true, false] , 
		["平板内的世界",          244, "",         "进入iCat", "-1/0/0",    "0/20/0", false, true, false] , 
		["爽滑慢舔",          244, "",         "所以为什么要吃苔藓？", "-1/0/0",    "0/42/0", true, true, true] , 
		["你个狗",          244, "",         "这真的比死水还阴，老阴逼", "-1/0/0",    "0/42/0", true, true, true] , 
		["干净的白客",          87, "",         "打开作者的调试面板", "-1/0/0",    "0/29/0", true, true, true] , 
		["反步兵地雷",          87, "",         "我的脚好痛！", "-1/0/0",    "0/35/0", true, true, true] , 
		["磕到甲沟炎了",          87, "",         "不是，怎么突然趋势了？", "-1/0/0",    "0/34/0", true, true, true] , 
		["通关",          87, "",         "通关此游戏", "-1/0/0",    "0/33/0", false, true, false] , 
		["外语骂人",           4, "",         "老太婆没学过英语，被你干沉默了",    "-1/0/0", "0/1/0",  true, true, true] 
