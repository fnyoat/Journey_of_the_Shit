...
(function() {
    ...
    (window as any).HitCoreConfig = hitCoreConfig;

    const _HitCore_Game_Action_makeDamageValue = Game_Action.prototype.makeDamageValue;
    Game_Action.prototype.makeDamageValue = function(target: any, critical: boolean): number {
        ...
        if (canBlock) {
            ...
            if (blockResult.defended) wasDefended = true;
        }
        ...
        if (canParry && finalValue > 0 && !wasDefended) {
            ...
            if (parryResult.defended) wasDefended = true;
        }
    ...
    Game_Action.prototype.applyBlock = function(target: any, damage: number, result: any, canDeflect: boolean): { damage: number, defended: boolean } {
        if (damage <= 0) return { damage, defended: false };
        
        const blockInfo = target.getBlockInfo();
        if (!blockInfo.hasBlockItem) return { damage, defended: false };
        
        const blockRate = blockInfo.blockRate / 100;
        if (Math.random() >= blockRate) return { damage, defended: false };
    ...
    Game_Action.prototype.applyParry = function(target: any, damage: number, result: any, canDeflect: boolean): { damage: number, defended: boolean } {
        if (damage <= 0) return { damage, defended: false };
        
        const parryInfo = target.getParryInfo();
        if (!parryInfo.hasWeapon) return { damage, defended: false };
        
        const parryRate = parryInfo.parryRate / 100;
        if (Math.random() >= parryRate) return { damage, defended: false };
    ...
    Game_Action.prototype.executeParryQTE = function(parryRate: number): number {
    ...
    Game_Action.prototype.applyDeflectDamage = function(damage: number, result: any, deflectInfo: { deflectRate: number, deflectValue: number }, reductionRate: number): number {
    ...
    Game_Action.prototype.applyIndependentDeflect = function(target: any, damage: number, result: any): number {
        if (damage <= 0) return damage;

        const deflectInfo = target.getIndependentDeflectInfo();
        const deflectRate = deflectInfo.deflectRate / 100;
        if (deflectRate <= 0 || Math.random() >= deflectRate) return damage;
    ...
    function parseBlockTag(tag: string): { rate: number, value: number, deflectRate: number, deflectValue: number } | null {
        if (!tag) return null;
    ...
    function parseDeflectTag(tag: string): { rate: number, value: number } | null {
        if (!tag) return null;
    ...
    Game_Battler.prototype.getBlockInfo = function(): { hasBlockItem: boolean, blockRate: number, blockValue: number, deflectRate: number, deflectValue: number } {
        ...
        if ((this as any).enemy) {
            const enemyMeta = (this as any).enemy().meta;
            if (enemyMeta['Block']) {
    ...
    function parseParryTag(tag: string): { rate: number, value: number, deflectRate: number, deflectValue: number } | null {
        if (!tag) return null;
        const nums = tag.split(',').map(Number);
        if (nums.length >= 2 && !isNaN(nums[0]) && !isNaN(nums[1])) {
            return {
    ...
    Game_Battler.prototype.getParryInfo = function(): { hasWeapon: boolean, parryRate: number, parryValue: number, deflectRate: number, deflectValue: number } {
        let hasWeapon = false;
    ...
    Game_Battler.prototype.getSecondaryDeflectInfo = function(): { deflectRate: number, deflectValue: number } {
        let totalDeflectRate = 0;
        let totalDeflectValue = 0;
    ...
    Game_Battler.prototype.getIndependentDeflectInfo = function(): { deflectRate: number, deflectValue: number } {
    ...
    Game_Battler.prototype.hasStateTag = function(tag: string): boolean {
    ...
    Game_Battler.prototype.hasParryQTE = function(): boolean {
        if (this.states) {
            for (const state of this.states()) {
        ...
        if ((this as any).enemy) {
            if ((this as any).enemy().meta['EnableParryQTE']) {
                return true;
            }
        }
        return false;
    ...
    Game_ActionResult.prototype.initialize = function(): void {
    ...
    Game_ActionResult.prototype.clear = function(): void {
