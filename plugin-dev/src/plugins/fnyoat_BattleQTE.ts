...
if (!Imported.fnyoat_CorePhysics) {
    fnyoat.PhysicsObject = function(x: number = 0, y: number = 0, config: any = {}) {
        this.x = x;
        this.y = y;
...
fnyoat.QTE.getParam = function(name: string) {
...
fnyoat.QTE.loadIndicatorConfigs = function() {
    ...
    for (let i = 1; i <= 10; i++) {
        ...
        if (!paramStr || paramStr === '{}' || paramStr.length < 10) continue;
        try {
...
fnyoat.QTE.registerIndicator = function(config: any) {
    if (!config.name || !config.key) { console.warn('fnyoat_BattleQTE: 注册指示器需要名称和按键'); return false; }
    config.key = Number(config.key) || 90;
...
fnyoat.QTE.registerGroup = function(config: any) {
    if (!config.name) { console.warn('fnyoat_BattleQTE: 注册QTE组需要名称'); return false; }
    config.chance = Number(config.chance) || 100;
    config.times = Number(config.times) || 1;
    config.speed = Number(config.speed) || this._params.defaultGroupSpeed;
    config.direction = config.direction || this._params.defaultGroupDirection;
...
fnyoat.QTE.getIndicator = function(name: string) { return this._indicatorConfigs[name]; };
fnyoat.QTE.getGroup = function(nameOrId: any) {
    if (typeof nameOrId === 'number') { return this._groups[nameOrId]; }
    return this._groups[nameOrId];
};

fnyoat.QTE.getWeaponAutoGroup = function(subject: any) {
    if (!subject || !subject.isActor()) return null;
...
fnyoat.QTE._checkGroupTag = function(note: string, subject: any) {
    let numMatch = note.match(/<FNQTE_Group:(\d+)>/);
    if (numMatch) { return { enabled: true, note: note }; }
...
fnyoat.QTE.Indicator = function(config: any) {
...
fnyoat.QTE.Indicator.prototype.update = function(judgmentArea: any) {
    ...
    if (this.physics) {
        ...
        if (this.hitType === 'perfect') { this.applyPerfectColor(); }
        if (this.physics.isGone()) { this.physics = null; } else { return; }
    }
    if (!this.active || this.hit || this.passed) { return; }
    if (fnyoat.QTE._startDelay && fnyoat.QTE._startDelay > 0) { return; }
...
fnyoat.QTE.Indicator.prototype.updateNormalMotion = function(judgmentArea: any) {
    const targetX = judgmentArea.x + judgmentArea.width / 2;
    const distance = Math.abs(this.x - targetX);
    let speedMod = 1;
    if (this.magnetic && distance < 150) { speedMod *= (1 - this.magneticStrength * (1 - distance / 150)); }
    if (this.antiMagnetic && distance < 100) { speedMod *= (1 + this.antiMagneticStrength * (1 - distance / 100)); }
    ...
    switch (this.direction) {
        case 'rightToLeft': this.x -= actualSpeed * (this.reversed ? -1 : 1); break;
        case 'leftToRight': this.x += actualSpeed * (this.reversed ? -1 : 1); break;
        case 'topToBottom': this.y += actualSpeed * (this.reversed ? -1 : 1); break;
        case 'bottomToTop': this.y -= actualSpeed * (this.reversed ? -1 : 1); break;
    }
};

fnyoat.QTE.Indicator.prototype.updateAdvancedMotion = function() {
    if (typeof this.advancedMotion === 'function') { this.advancedMotion(this, this.currentTime); }
};

fnyoat.QTE.Indicator.prototype.checkPassed = function(judgmentArea: any) {
    ...
    switch (this.direction) {
        case 'rightToLeft':
            if (!this.reversed && centerX < judgeLeft - this.width) { this.handlePass(); }
            else if (this.reversed && centerX > judgeRight + this.width) { this.handlePass(); }
            break;
        case 'leftToRight':
            if (!this.reversed && centerX > judgeRight + this.width) { this.handlePass(); }
            else if (this.reversed && centerX < judgeLeft - this.width) { this.handlePass(); }
            break;
    }
};
...
fnyoat.QTE.Indicator.prototype.handlePass = function() {
    ...
    if (this.currentTimes >= this.times) {
        if (!this.hitType) { this.hitType = 'miss'; }
        this.passed = true;
        this.startFadeOut();
...
fnyoat.QTE.Indicator.prototype.resetPosition = function() {
    switch (this.direction) {
        case 'rightToLeft': this.x = this.reversed ? -this.width / 2 : Graphics.width + this.width / 2; break;
        case 'leftToRight': this.x = this.reversed ? Graphics.width + this.width / 2 : -this.width / 2; break;
    }
    this.currentTime = 0;
    this.currentSpeed = this.speed;
};
...
fnyoat.QTE.Indicator.prototype.calculateAccuracy = function(judgmentArea: any) {
    ...
    if (dx > judgeHalfWidth || dy > judgeHalfHeight) { return 0; }
...
fnyoat.QTE.Indicator.prototype.checkHit = function(judgmentArea: any) {
    if (this.passed || this.hit) return false;
    this.accuracy = this.calculateAccuracy(judgmentArea);
    if (this.accuracy <= 0) { this.hit = false; this.hitType = 'miss'; return false; }
    if (this.accuracy >= 95) { this.hit = true; this.hitType = 'perfect'; return true; }
    this.hit = true; this.hitType = 'success'; return true;
};

fnyoat.QTE.StarParticle = function(x: number, y: number) {
    this.x = x; this.y = y;
    ...
    this.opacity = 1; this.gone = false;
};
...
fnyoat.QTE.StarParticle.prototype.draw = function(bitmap: any) {
    if (this.gone) return;
    const cx = this.x; const cy = this.y; const r = this.size;
...
fnyoat.QTE.StarParticle.prototype.drawStar = function(ctx: any, cx: number, cy: number, outerR: number, innerR: number, points: number, fillColor: string, strokeColor: string) {
    ctx.beginPath();
...
fnyoat.QTE.start = function(action: any) {
    const enableQTE = (this._params.enableQTE === true || this._params.enableQTE === 'true');
    if (!enableQTE || !this._active) return false;
    ...
    if (!qteData.enabled) return false;
    if (qteData.chance < 100 && Math.randomInt(100) >= qteData.chance) { return false; }
    this._indicators = this.createIndicators(qteData);
    if (this._indicators.length === 0) return false;
...
fnyoat.QTE.initIndicatorPositions = function() {
    this._indicators.forEach(function(indicator: any) {
        switch (indicator.direction) {
            case 'rightToLeft': indicator.x = Graphics.width + indicator.width / 2; break;
            case 'leftToRight': indicator.x = -indicator.width / 2; break;
            case 'topToBottom': indicator.y = -indicator.height / 2; break;
            case 'bottomToTop': indicator.y = Graphics.height + indicator.height / 2; break;
        }
        indicator.y = this._judgmentArea.y + this._judgmentArea.height / 2;
    }, this);
};

fnyoat.QTE.parseQteData = function(action: any, item: any) {
    let note = ''; let enabled = false; let isDodge = false;
    ...
    if (isAttackAction && subject && subject.isActor()) {
        if (subject.actor() && subject.actor().note) {
            ...
            if (result.enabled) { note = result.note; enabled = true; }
        }
        ...
        if (!enabled) {
            ...
            for (let i = 0; i < armors.length; i++) {
                ...
                if (armor && armor.note) {
                    note = armor.note;
                    const result = this._checkGroupTag(note, subject);
                    if (result.enabled) { note = result.note; enabled = true; break; }
                }
            }
        }
        if (!enabled) {
        ...
        if (!enabled && item && item.note) {
            note = item.note;
            if (note.match(/<FNQTE_Group:(\d+)>/)) { enabled = true; }
            if (note.match(/<FNQTE_Dodge_Group:(\d+)>/)) { enabled = true; isDodge = true; }
        }
    } else {
        if (item && item.note) {
            note = item.note;
    ...
    const data: any = { enabled: true, isDodge: isDodge, groupName: null, indicatorNames: null, chance: 100, width: null, speed: null, reverse: false, times: null, magnetic: null, accel: null, hallucination: false, damageSegments: 1, segmentIndicators: [], realTimeMode: true, interval: 0 };
    if (note.match(/<FNQTE_Group:(\d+)>/)) { data.groupName = parseInt(RegExp.$1); }
    if (note.match(/<FNQTE_Dodge_Group:(\d+)>/)) { data.groupName = parseInt(RegExp.$1); }
    if (note.match(/<FNQTE_Chance:(\d+)>/)) { data.chance = parseInt(RegExp.$1); }
    if (note.match(/<FNQTE_Width:(\d+)>/)) { data.width = parseInt(RegExp.$1); }
    if (note.match(/<FNQTE_Speed:([\d.]+)>/)) { data.speed = parseFloat(RegExp.$1); }
    if (note.includes('<FNQTE_Reverse>')) { data.reverse = true; }
    if (note.match(/<FNQTE_Times:(\d+)>/)) { data.times = parseInt(RegExp.$1); }
    if (note.match(/<FNQTE_Magnetic:([\d.]+)>/)) { data.magnetic = parseFloat(RegExp.$1); }
    if (note.match(/<FNQTE_Accel:([\d.]+)>/)) { data.accel = parseFloat(RegExp.$1); }
    if (note.includes('<FNQTE_Hallucination>')) { data.hallucination = true; }
    if (note.match(/<FNQTE_DamageSegments:(\d+)>/)) { data.damageSegments = parseInt(RegExp.$1); }
    if (note.match(/<FNQTE_Segments_Indicators:(.+)>/)) {
        const segmentStr = RegExp.$1;
        const segments = segmentStr.split(',');
        for (let i = 0; i < segments.length; i++) { data.segmentIndicators.push(parseInt(segments[i].trim())); }
    }
    if (note.includes('<FNQTE_RealTime>')) { data.realTimeMode = true; }
    if (note.includes('<FNQTE_NoRealTime>')) { data.realTimeMode = false; }
    return data;
};

fnyoat.QTE.createIndicators = function(qteData: any) {
    const indicators = []; let indicatorList = [];
    if (qteData.groupName) {
        ...
        if (group) {
            ...
            for (let i = 0; i < names.length; i++) {
                ...
                if (config) { indicatorList.push(config); }
            }
        }
    ...
    let segmentIndex = 0; let currentSegmentIndicator = 0;
    ...
    for (let i = 0; i < indicatorList.length; i++) {
        ...
        if (segmentIndicators.length > 0 && currentSegmentIndicator >= segmentIndicators[segmentIndex]) {
            segmentIndex++; currentSegmentIndicator = 0;
            if (segmentIndex < segmentIndicators.length) { indicatorsPerSegment = segmentIndicators[segmentIndex]; }
...
fnyoat.QTE.getReverseDirection = function(dir: string) {
    switch (dir) {
...
fnyoat.QTE.getBattlerSprite = function(battler: any) {
    ...
    if (spriteset && spriteset._battlerSprites) {
        for (let i = 0; i < spriteset._battlerSprites.length; i++) {
            ...
            if (sprite && sprite._battler === battler) { return sprite; }
        }
    }
    return null;
};
...
fnyoat.QTE.setupJudgmentArea = function() {
    ...
    let targets = null;
    if (this._currentAction && typeof this._currentAction.targets === 'function') { targets = this._currentAction.targets(); }
    ...
    if (sprite) {
        ...
        switch (position) {
            case 'left':
                ...
                this._judgmentArea.y = sprite.y - this._judgmentArea.height / 2; break;
            case 'right':
                this._judgmentArea.x = sprite.x + sprite.width + 50;
                this._judgmentArea.y = sprite.y - this._judgmentArea.height / 2; break;
            case 'top':
                this._judgmentArea.x = sprite.x - this._judgmentArea.width / 2;
                this._judgmentArea.y = sprite.y - this._judgmentArea.height - 50; break;
            case 'bottom':
                this._judgmentArea.x = sprite.x - this._judgmentArea.width / 2;
                this._judgmentArea.y = sprite.y + sprite.height + 50; break;
            case 'center':
                this._judgmentArea.x = Graphics.width / 2 - this._judgmentArea.width / 2;
                this._judgmentArea.y = Graphics.height / 2 - this._judgmentArea.height / 2; break;
        }
    } else {
        this._judgmentArea.x = Graphics.width / 2 - this._judgmentArea.width / 2;
...
fnyoat.QTE.createSprites = function() {
    ...
    this._indicators.forEach(function(indicator: any, index: number) {
    ...
    this._sprites.stars.x = 0; this._sprites.stars.y = 0;
    this._sprites.container.addChild(this._sprites.stars);
};

fnyoat.QTE.drawStrokeRect = function(bitmap: any, x: number, y: number, width: number, height: number, color: string, lineWidth: number) {
...
fnyoat.QTE.drawFillCircle = function(bitmap: any, x: number, y: number, radius: number, color: string) {
    const oldAlpha = bitmap.context.globalAlpha;
    bitmap.context.globalAlpha = oldAlpha;
...
fnyoat.QTE.drawJudgmentArea = function() {
    if (!this._sprites || !this._sprites.judgment) return;
    ...
    const centerX = area.width / 2; const centerY = area.height / 2;
...
fnyoat.QTE.handleKeyPress = function(keyCode: number) {
    if (!this._processing || !this._judgmentArea || !this._sprites) return;
    if (this._handlingKey) return;
...
fnyoat.QTE.spawnPerfectHitStars = function() {
    ...
    if (!indicator || !indicator.sprite) return;
...
fnyoat.QTE.endQTE = function() {
    ...
    if (this._result.hitCount > 0) { this._result.success = true; }
    ...
    (BattleManager as any)._qteResult = result;
    this._activeIndicatorIndex = 0;
    this._intervalTimer = 0;
};
...
fnyoat.QTE.setActive = function(active: boolean) {
    this._active = active;
};
...
fnyoat.QTE.drawIndicator = function(bitmap: any, indicator: any) {
    bitmap.clear();
    if (this._params.glowEffect) { bitmap.fillRect(-5, -5, bitmap.width + 10, bitmap.height + 10, this._params.glowColor); }
    ...
    const centerX = bitmap.width / 2; const centerY = bitmap.height / 2;
...
fnyoat.QTE.update = function() {
    if (this._cleaningUp) {
        ...
        this._indicators.forEach(function(indicator: any) {
            if (indicator.physics) {
                ...
                if (indicator.physics) { allDone = false; }
            }
        }, this);
        if (this._stars.length > 0) { allDone = false; }
        this._stars.forEach(function(star: any) { star.update(); });
        this._stars = this._stars.filter(function(star: any) { return !star.gone; });
        if (this._sprites && this._sprites.stars) {
            const starsBitmap = this._sprites.stars.bitmap;
            starsBitmap.clear();
            this._stars.forEach(function(star: any) { star.draw(starsBitmap); });
            starsBitmap._dirty = true;
        }
        if (allDone) { this._cleaningUp = false; this.removeSprites(); }
        return;
    }
    if (!this._processing || !this._judgmentArea || !this._sprites) return;
    ...
    if (this._params.enableAlertSound && this._alertTimer >= this._params.alertInterval) { this._alertTimer = 0; }
    this._indicators.forEach(function(indicator: any) { indicator.update(this._judgmentArea); }, this);
    const currentIndicator = this._indicators[this._activeIndicatorIndex];
    if (currentIndicator) { if (currentIndicator.passed && !currentIndicator.hit) { this.nextIndicator(); } }
    this.checkNextIndicator();
    if (this._perfectHitFlash > 0) {
        this._perfectHitFlash--;
        if (this._perfectHitFlash > 0) { this.updatePerfectHitFlash(); } else { this.drawJudgmentArea(); }
    }
    this._stars.forEach(function(star: any) { star.update(); });
    this._stars = this._stars.filter(function(star: any) { return !star.gone; });
    if (this._sprites && this._sprites.stars) {
...
fnyoat.QTE.updateSprites = function() {
    if (!this._sprites || !this._sprites.indicators) return;
    this._sprites.indicators.forEach(function(sprite: any) {
...
Scene_Battle.prototype.onQTEKeyDown = function(event: any) {
...
const _Window_BattleLog_updateWaitMode = (Window_BattleLog.prototype as any).updateWaitMode;
(Window_BattleLog.prototype as any).updateWaitMode = function() {
    if (this._waitMode === 'qte') {
        if (fnyoat.QTE.isProcessing()) return true;
        this._waitMode = '';
    }
...
(BattleManager as any).prepareQTE = function() {
    const subject = this._subject;
    if (!subject) return;
    const action = subject.currentAction();
    if (!action) return;
    if (fnyoat.QTE._lastAction === action) return;
    const item = action.item();
    if (!item) return;
    const qteResult = fnyoat.QTE.start(action);
    if (qteResult) {
        (this as any)._qteWaiting = true;
        this._logWindow.setWaitMode('qte');
    }
};
BattleManager.startAction = function() {
    (BattleManager as any).prepareQTE.call(this);
    _BattleManager_startAction.call(this);
};

const _BattleManager_isBusy = (BattleManager as any).isBusy;
(BattleManager as any).isBusy = function() {
...
BattleManager.endAction = function() {
    ...
    (BattleManager as any)._qteResult = null;
    _BattleManager_endAction.call(this);
};

const _Game_Action_evalDamageFormula = (Game_Action.prototype as any).evalDamageFormula;
(Game_Action.prototype as any).evalDamageFormula = function(target: any) {
    const value = _Game_Action_evalDamageFormula.call(this, target);
    const qteResult = (BattleManager as any)._qteResult;
...
Game_Action.prototype.itemHit = function(target: any) {
...
Game_Action.prototype.calcDamage = function(target: any) {
...
Game_Interpreter.prototype.pluginCommand = function(command: string, args: string[]) {
    if (command.toLowerCase() === 'fnyoatqte') {
        ...
        switch (subCommand) {
            case 'start': fnyoat.QTE.setActive(true); break;
            case 'stop': fnyoat.QTE.stop(); break;
            case 'active': fnyoat.QTE.setActive(true); break;
            case 'inactive': fnyoat.QTE.setActive(false); break;
        }
    } else {
