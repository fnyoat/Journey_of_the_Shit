...
Graphics.render = function(stage) {
    if (this._skipCount === 0) {
        ...
        this._skipCount = Math.min(Math.floor(elapsed / 14), this._maxSkip);
        this._rendered = true;
    } else {
        this._skipCount--;
        this._rendered = false;
...
Graphics._createFPSMeter = function() {
    var options = { graph: 1, decimals: 0, theme: 'transparent', toggleOn: null, maxFps: 70 };
...
Html5Audio._startGainTween = function (duration) {
    ...
    this._tweenGainStep = (this._tweenTargetGain - this._tweenGain) / (70 * duration);
