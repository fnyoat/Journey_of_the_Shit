...
(function () {
    ...
    const SHARED_TYPE_IDS = parseSharedTypeIds();
    const sharedTypeSet = new Set(SHARED_TYPE_IDS);
    if (SHARED_TYPE_IDS.length < 2) {
        console.warn('[TwoAccessorySlots] 共享装备类型至少需要2个，插件未生效。');
        return;
    }
    // 判断某个装备类型ID是否属于共享槽
    function isSharedType(etypeId) {
     ...
     */
    function isSharedSlot(actor, slotId) {
     ...
     * 获取所有共享槽位的索引列表
     */
    function getSharedSlotIndices(actor) {
        const slots = actor.equipSlots();
        const indices = [];
    ...
    const fnyoatNS = window.fnyoat = window.fnyoat || {};
