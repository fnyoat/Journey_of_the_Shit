...
const fnyoat_g = window.fnyoat;
const ER_MAX = 30;
const ER_RUNS = 3;
const ER_FILENAME = 'error_report.log';
const ST = {
    errors: [],
    initialized: false,
    runId: 0,
...
fnyoat_g.ErrorReporter = {
    report: function (t, m, s, k) {
        _rep(t, m, s, k);
    },
    getAll: function () { return ST.errors.slice(); },
    getLatest: function () { return ST.errors.length ? ST.errors[ST.errors.length - 1] : null; },
    clear: function () { ST.errors = []; },
};
function _rep(title, message, stack, kind) {
    try {
        const e = {
            time: _ft(new Date()),
            runId: ST.runId,
...
function _init() {
    ...
    const win = window;
    if (win) {
        const old = win.onerror;
        win.onerror = function (msg, url, line, col, err) {
            try {
                const m = String(msg) + '  @ ' + (url || '?') + ':' + (line != null ? line : '?') + ':' + (col != null ? col : '?');
                const s = (err && err.stack) ? String(err.stack) : m;
                _rep('[window] Script Error', m, s, 'window');
            }
            catch (e) { }
            if (old) {
                try {
                    const r = old.apply(win, arguments);
                    if (r === true)
                        return true;
                }
                catch (e2) { }
    ...
    if (win && win.addEventListener) {
        win.addEventListener('error', function (ev) {
            try {
                const err = ev.error;
                const m = err ? String(err.message) + '  @ ' + (err.fileName || '?') + ':' + (err.lineNumber != null ? err.lineNumber : '?') + ':' + (err.columnNumber != null ? err.columnNumber : '?') : String(ev.message || ev);
                const s = (err && err.stack) ? String(err.stack) : m;
        ...
        win.addEventListener('unhandledrejection', function (ev) {
            try {
                ...
                const s = (reason && reason.stack) ? String(reason.stack) : m;
...
function _hookRM() {
    try {
        ...
        if (G && G.printError && !G._er_hooked) {
            const orig = G.printError;
            G.printError = function (name, msg) {
                try {
                    _rep('[RMMV] Graphics.printError: ' + String(name), String(msg), '', 'rmmv');
                }
                catch (e) { }
                try {
                    orig && orig.call(this, name, msg);
                }
                catch (e2) { }
            };
            G._er_hooked = true;
        ...
        if (SM && SM.updateMain && !SM._er_hooked_upd) {
            const orig = SM.updateMain;
            SM.updateMain = function () {
                try {
                    orig.call(this);
                }
                catch (err) {
        ...
        if (DM && DM.loadData && !DM._er_hooked_ld) {
            const orig = DM.loadData;
            DM.loadData = function (cls, src) {
                try {
        ...
        if (!ST.hooked)
            setTimeout(function () { _hookRM(); }, 1000);
    }
}
function _watchCrash() {
    const win = window;
...
function _purgeOldRuns() {
    try {
        ...
        const fn = ER_FILENAME;
        const fp = pathMod && pathMod.join ? pathMod.join(dir, fn) : (dir.replace(/\\$/, '') + '/' + fn);
        if (!fs.existsSync(fp))
            return;
        ...
        if (runStarts.length <= ER_RUNS)
            return;
        const cutoffIndex = runStarts[runStarts.length - ER_RUNS];
        const newContent = lines.slice(cutoffIndex).join('\n');
...
function _file(entry) {
    try {
        ...
        const fn = ER_FILENAME;
        const fp = pathMod && pathMod.join
            ? pathMod.join(dir, fn)
            : (dir.replace(/\\$/, '') + '/' + fn);
        _purgeOldRuns();
        const line = '========== [' + entry.time + '] [' + entry.kind + '] ' + entry.title + ' (RunID=' + entry.runId + ') ==========\n' +
            entry.message + '\n' +
            (entry.stack ? entry.stack + '\n' : '') + '\n';
        if (fs.appendFileSync)
            fs.appendFileSync(fp, line, 'utf8');
        else if (fs.appendFile)
            fs.appendFile(fp, line, 'utf8', function () { });
    }
    catch (e) { }
}
try {
...
fnyoat_g.ErrorReporter.test = function () {
