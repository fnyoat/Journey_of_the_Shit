...
(function () {
    ...
    function getHateMultiplier(battler) {
    ...
    function hasTauntState(battler) {
        if (!battler || !battler.states)
            return false;
        for (const state of battler.states()) {
    ...
    function getFocusFireMultiplier(battler) {
        if (!battler || !battler.states)
            return 1;
        for (const state of battler.states()) {
    ...
    class HateSystem {
        ...
        getEnemyHate(enemyIndex) {
            return this._enemyHate.get(enemyIndex) || null;
        }
        getHate(enemyIndex, actorId) {
        ...
        setHate(enemyIndex, actorId, value) {
            const hateList = this.getEnemyHate(enemyIndex);
            if (!hateList)
                return;
            const entry = hateList.find(e => e.actorId === actorId);
        ...
        addHate(enemyIndex, actorId, value, type = 'damage') {
            const hateList = this.getEnemyHate(enemyIndex);
            if (!hateList)
                return;
            const entry = hateList.find(e => e.actorId === actorId);
        ...
        isTaunted(enemyIndex) {
            return this._tauntedEnemies.has(enemyIndex);
        }
        copyHateToTop(sourceActorId, targetEnemyIndex) {
        ...
        addDamageDealt(actorId, amount) {
            const stat = this._battleStats.get(actorId);
            if (stat) {
                stat.damageDealt += amount;
            }
        }
        addDamageTaken(actorId, amount) {
            const stat = this._battleStats.get(actorId);
            if (stat) {
                stat.damageTaken += amount;
            }
        }
        addHealingDone(actorId, amount) {
            const stat = this._battleStats.get(actorId);
            if (stat) {
        ...
        selectTargetWithHate(enemyIndex) {
            const hateList = this.getEnemyHate(enemyIndex);
            if (!hateList || hateList.length === 0)
                return -1;
            ...
            if (aliveActors.length === 0)
                return -1;
            const hateListFiltered = hateList.filter(e => aliveActors.some(a => a.actorId() === e.actorId));
            if (hateListFiltered.length === 0)
                return -1;
    ...
    window.fnyoat.HateSystem = {
        getEnemyHate: (enemyIndex) => hateSystem.getEnemyHate(enemyIndex),
        getHate: (enemyIndex, actorId) => hateSystem.getHate(enemyIndex, actorId),
        setHate: (enemyIndex, actorId, value) => hateSystem.setHate(enemyIndex, actorId, value),
        addHate: (enemyIndex, actorId, value, type) => hateSystem.addHate(enemyIndex, actorId, value, type),
        taunt: (enemyIndex, actorId) => hateSystem.taunt(enemyIndex, actorId),
        isTaunted: (enemyIndex) => hateSystem.isTaunted(enemyIndex),
        copyHateToTop: (sourceActorId, targetEnemyIndex) => hateSystem.copyHateToTop(sourceActorId, targetEnemyIndex),
        resetAll: () => hateSystem.resetAll(),
        getBattleStats: () => hateSystem.getBattleStats(),
        selectTargetWithHate: (enemyIndex) => hateSystem.selectTargetWithHate(enemyIndex),
    ...
    const _Hate_Game_Action_executeHpDamage = Game_Action.prototype.executeHpDamage;
