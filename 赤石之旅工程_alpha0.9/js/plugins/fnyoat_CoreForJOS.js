...
const isNode = (typeof require !== 'undefined' && typeof module !== 'undefined') || typeof window.nw !== 'undefined';
if (isNode) {
    const os = require('os');
    const v8 = require('v8');
    ...
    for (let i = 0; i < ifaceValues.length; i++) {
        const arr = ifaceValues[i];
        for (let j = 0; j < arr.length; j++) {
            const info = arr[j];
            if (info.family === 'IPv4' && !info.internal)
                netAddrs.push(info.address);
        }
    }
    const sysInfo = {
...
(function () {
    ...
    function _getPath() {
        ...
        try {
            ...
            if (proc && proc.env) {
                ...
                if (base)
                    return base + '/' + APP_NAME + '/' + STORE_FILENAME;
            }
        }
        catch (e) { /* ignore */ }
        return null;
    }
    function _readFileStore() {
        const path = _getPath();
        if (!path)
            return null;
        try {
    ...
    function _save(data) {
        ...
        if (!ok)
            _writeLS(data); // 文件写入失败时，回退到 localStorage
    }
    // ============================================================
    ...
    if (typeof fnyoat !== 'undefined') {
        ...
        storeApi.all = function () {
            return _load() || {};
        };
        storeApi.clear = function () {
            _save({});
        };
        storeApi.path = function () { return _getPath(); };
        fnyoat.store = storeApi;
        // ============================================================
        ...
        (function () {
            ...
            function _showTUIMenu() {
                return new Promise(function (resolve) {
                    const options = _tuiOptions.map(function (opt) {
                    ...
                    fnyoat.Terminal.menu(options, undefined, '─── TUI 模式 ───', '  ↑↓ 选择   Enter 确认   Esc 返回上级', initialSelected).then(function (idx) {
            ...
            function _showMainMenu() {
                ...
                ], _josOnExit).then(function (choice) {
            ...
            // ---------- TUI 读取界面 → Scene_Load ----------
            function _doTUILoad(slotIndex) {
                if (slotIndex < 0) {
                    nextTUI();
                    return;
                }
                fnyoat.Terminal.close();
                if (typeof DataManager !== 'undefined' && DataManager.drill_GSM_doLoad) {
                    if (DataManager.drill_GSM_doLoad(slotIndex)) {
                    ...
                    if (DataManager.selectSavefileForNewGame) {
            ...
            function _showTUISettings() {
                ...
                function enterVolumeScreen() {
                    ...
                    function render() {
                        ...
                        for (let i = 0; i < volItems.length; i++) {
                            const val = window.ConfigManager && window.ConfigManager[volItems[i].key] !== undefined
                                ? window.ConfigManager[volItems[i].key]
                                : 100;
                ...
                function enterToggleScreen() {
                    ...
                    function render() {
                        ...
                        for (let i = 0; i < toggleItems.length; i++) {
                            const CM = window.ConfigManager;
                            let val = true;
                            if (CM && CM[toggleItems[i].key] !== undefined)
                                val = CM[toggleItems[i].key];
                ...
                fnyoat.Terminal.menu([
                    ...
                    if (idx === 0)
                        ...
                        nextTUI(); // 返回（idx === 2 或 -1）
                });
            }
            // ---------- TUI 鸣谢名单 ----------
            // 居中辅助：终端约38字符宽
            function credit(text, cls) {
                const W = 38;
                const pad = Math.max(0, Math.floor((W - text.length) / 2));
                fnyoat.Terminal.println(' '.repeat(pad) + text, cls);
            }
            function _showTUICredits() {
                // 禁用自动滚底，让内容从头开始显示
                fnyoat.Terminal.setAutoScroll(false);
                credit('─── 鸣谢名单 ───', 't-info');
                ...
                fnyoat.Terminal.setWaitAnyKey(function () {
            ...
            function nextTUI() {
                _showTUIMenu().then(function (id) {
                    ...
                    if (id === 'load') {
                        const maxSlots = (typeof DataManager !== 'undefined' && DataManager.maxSavefiles)
                            ? DataManager.maxSavefiles() : 20;
                        fnyoat.Terminal.showLoadScreen(maxSlots).then(function (slot) {
                            _doTUILoad(slot);
                        });
                        return;
                    }
            ...
            fnyoat.JOS = {
                start: function () {
                    ...
                    fnyoat.Terminal.setOnCloseCallback(function () {
                    ...
                    fnyoat.Terminal.setJosMenuHandler(function () {
                        fnyoat.Terminal.clear();
                        fnyoat.Terminal.println(fnyoat.getJOSLogo());
                        fnyoat.Terminal.newLine();
