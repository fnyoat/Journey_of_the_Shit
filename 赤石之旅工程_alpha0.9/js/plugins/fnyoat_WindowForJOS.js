...
//=============================================================================
fnyoat.openGameRatingWindow = function () {
    try {
        let win;
        let foundMethod = '';
        try {
            ...
            try {
                ...
                throw new Error('无法找到Window API: ' + e.message + ', ' + e2.message);
            }
        }
...
<div style="padding: 15px 20px;">
    ...
    <div style="display: flex; justify-content: center; gap: 20px;">
        ...
        <button id="cancelBtn" style="width: 75px; height: 30px; background: linear-gradient(180deg, #f5f5f5 0%, #e0e0e0 50%, #d0d0d0 100%); border: 1px solid #999; border-radius: 3px; font-size: 12px; cursor: pointer;">取消</button>
                ...
                const ratingInput = winDocument.getElementById('ratingInput');
        ...
        console.log('HTML窗口不可用，使用备用方案: ' + e.message);
...
// 获取评分
fnyoat.getLastGameRating = function () {
    return fnyoat.lastGameRating || 0;
};
//=============================================================================
// 同意请求弹窗功能 - 显示权限请求对话框
//=============================================================================
fnyoat.openPermissionRequestWindow = function (requesterId, requesterName, resourceName) {
    try {
        let win;
        let foundMethod = '';
        try {
...
// 获取权限请求结果
fnyoat.getLastPermissionResult = function () {
