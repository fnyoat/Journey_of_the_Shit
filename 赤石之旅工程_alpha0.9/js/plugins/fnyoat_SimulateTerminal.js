...
(function () {
    ...
    function _freezeGame() {
        _gameFreezeDepth++;
    }
    function _unfreezeGame() {
        if (_gameFreezeDepth > 0)
            _gameFreezeDepth--;
    }
    function _isGameFrozen() {
        return _gameFreezeDepth > 0;
    }
    // 全局挂载，供闭包内各函数调用
    window.__fnyoat_terminalFreeze = _freezeGame;
    window.__fnyoat_terminalUnfreeze = _unfreezeGame;
    window.__fnyoat_isTerminalFrozen = _isGameFrozen;
...
(function () {
    ...
    let _output = null;
    let _inputRow = null;
    let _promptSpan = null;
    let _inputSpan = null;
    let _cursorSpan = null;
    let _inputAfterSpan = null;
    let _pasteInput = null;
    let _cursorVisible = true;
    let _cursorTimer = 0;
    let _cursorBlinkRate = 500;
    let _cursorTickerId = 0;
    let _inputBuffer = '';
    let _cursorPos = 0;
    ...
    let _commands = Object.create(null);
    let _currentUser = 'player'; // 当前登录用户，默认为 player
    // login 状态机：null = 空闲；'username' = 等待输入用户名；'password' = 等待输入密码
    let _loginState = null;
    let _josMenuHandler = null; // jos 命令：打开主菜单的回调
    let _closeResolve = null; // open() 返回的 Promise 的 resolve
    let _onCloseCallback = null; // 终端关闭时的回调
    let _sudoCache = Object.create(null); // sudo 缓存：用户 -> 过期时间戳
    const _ROOT_PASSWORD = 'root';
    let _suState = null; // su 状态：等待密码输入
    let _sudoState = null; // sudo 状态：等待密码输入
    ...
    // TUI 读取界面状态
    let _loadScreenState = null;
    let _loadSelectedEl = null; // 选中槽的 DOM 元素，用于 scrollIntoView
    // TUI 读取界面：按键重复节流（解决长按时卡顿）
    let _loadRepeatTimer = null;
    let _loadRepeatDir = 0; // 1 = 向下，-1 = 向上
    let _loadRepeatStop = false; // keyup 后软停止：阻止 interval 再次触发移动
    let _loadRepeatKeyDownTime = 0; // 记录上次 keydown 时间，防止旧 timer 触发
    ...
    function _getRealGameRoot() {
        // 优先尝试 NW.js process.cwd()
        try {
            const proc = window.process;
            if (proc && typeof proc.cwd === 'function') {
                const cwd = proc.cwd();
                if (cwd && cwd.length > 2 && cwd.charAt(1) === ':')
                    return cwd;
            }
        }
        catch (e) { /* ignore */ }
        ...
        try {
            const gui = window.require ? window.require('nw.gui') : null;
            if (gui && gui.App && gui.App.startPath) {
                const p = gui.App.startPath;
                if (p && p.length > 0)
                    return p;
            }
        }
    ...
    const _realGameRoot = _getRealGameRoot();
    // 虚拟路径 -> 真实文件系统路径
    function _vfsToReal(virtualPath) {
        if (!_realGameRoot)
            return null;
        const v = _normalizeVirtualPath(virtualPath);
        // 游戏根目录本身 -> 真实游戏根目录
        if (v === _virtualGameRoot)
            return _realGameRoot;
        ...
        if (/^(CON|PRN|AUX|NUL|CONIN\$|CONOUT\$|COM[1-9]|LPT[1-9])(\.|;|:|$)/.test(filename))
            return null;
        return real;
    }
    // ---------- 真实磁盘操作（仅限游戏根目录下） ----------
    function _realFsModule() {
        return window.require ? window.require('fs') : null;
    }
    function _realIsDir(absVirtual) {
        const real = _vfsToReal(absVirtual);
        if (!real)
            return false;
        try {
            const fs = _realFsModule();
    ...
    function _realIsFile(absVirtual) {
        const real = _vfsToReal(absVirtual);
        if (!real)
            return false;
        try {
    ...
    function _realGetSize(absVirtual) {
        const real = _vfsToReal(absVirtual);
        if (!real)
            return -1;
        try {
    ...
    function _realExists(absVirtual) {
        const real = _vfsToReal(absVirtual);
        if (!real)
            return false;
        try {
    ...
    function _realListDir(absVirtual) {
        const real = _vfsToReal(absVirtual);
        if (!real)
            return [];
        try {
    ...
    function _realReadFile(absVirtual) {
        const real = _vfsToReal(absVirtual);
        if (!real)
            return null;
        try {
    ...
    function _vfsIsDir(absVirtual) {
        if (_isOutsideGameRoot(absVirtual))
            return false;
        return _realIsDir(absVirtual);
    }
    function _vfsIsFile(absVirtual) {
        if (_isOutsideGameRoot(absVirtual))
            return false;
        return _realIsFile(absVirtual);
    }
    function _vfsExists(absVirtual) {
        if (_isOutsideGameRoot(absVirtual))
            return false;
        return _realExists(absVirtual);
    }
    function _vfsListDir(absVirtual) {
        if (_isOutsideGameRoot(absVirtual))
            return [];
        return _realListDir(absVirtual);
    }
    function _vfsReadFile(absVirtual) {
        if (_isOutsideGameRoot(absVirtual))
            return null;
        return _realReadFile(absVirtual);
    }
    function _vfsJoinDir(absDir, name) {
    ...
    let _savedHtmlStyle = '';
    let _savedBodyStyle = '';
    let _savedBodyChildrenHidden = [];
    // ---------- 样式表 —— 只注入一次 ----------
    function _injectStyle() {
        if (document.getElementById('fnyoat-terminal-style'))
            return;
    ...
    function _smartScroll() {
        ...
        requestAnimationFrame(function () {
            ...
            const h = Math.max(document.documentElement.scrollHeight || 0, document.body ? document.body.scrollHeight : 0);
            window.scrollTo(0, h);
        });
    }
    function _refreshInputRow() {
        if (!_inputRow || !_promptSpan || !_inputSpan || !_cursorSpan)
            return;
    ...
    function open() {
        ...
        window.__fnyoat_terminalFreeze();
        ...
        for (let i = 0; i < bodyEl.children.length; i++) {
            ...
            if (child.id === 'fnyoat-terminal-style')
                continue;
            _savedBodyChildrenHidden.push({ el: child, display: child.style.display });
            child.style.display = 'none';
        }
        _buildDom();
        _history = (typeof fnyoat !== 'undefined' && fnyoat.store) ? (fnyoat.store.get('shell_history') || []) : [];
    ...
    function close() {
        ...
        if (_menuState) {
            if (_menuState.timer)
                clearInterval(_menuState.timer);
        ...
        if (_savedHtmlStyle === '')
            ...
            htmlEl.setAttribute('style', _savedHtmlStyle);
        if (_savedBodyStyle === '')
            bodyEl.removeAttribute('style');
        else
            bodyEl.setAttribute('style', _savedBodyStyle);
        ...
        window.__fnyoat_terminalUnfreeze();
    ...
    function undoLines(count) {
        ...
        for (let i = 0; i < n; i++) {
            const last = _output.lastElementChild;
            if (last)
                _output.removeChild(last);
            else
                break;
        }
    }
    function info(text) { if (!_isOpen)
        return; _appendLine(text, 't-info'); }
    function success(text) { if (!_isOpen)
        return; _appendLine(text, 't-success'); }
    function warn(text) { if (!_isOpen)
        return; _appendLine(text, 't-warn'); }
    function error(text) { if (!_isOpen)
        return; _appendLine(text, 't-error'); }
    // ---------- 中断 ----------
    function interrupt() {
        if (!_isOpen)
            return;
    ...
    function _runInterruptCallbacks() {
    ...
    function run(executor) {
        return new Promise(function (resolve) {
            ...
            _refreshInputRow();
            const ctx = {
                println: function (text) { _appendLine(text == null ? '' : String(text)); },
                write: function (text) { _writeInline(text); },
                newLine: function () { _appendLine(''); },
                clear: function () { clear(); },
                info: function (text) { _appendLine(text, 't-info'); },
                success: function (text) { _appendLine(text, 't-success'); },
                warn: function (text) { _appendLine(text, 't-warn'); },
                error: function (text) { _appendLine(text, 't-error'); },
                sleep: _ctxSleep,
                yield: function () { return _ctxSleep(16); },
                isInterrupted: function () { return _interruptRequested; },
                onInterrupt: function (fn) { _interruptCallbacks.push(fn); },
            };
            let maybePromise;
            try {
    ...
    function _finalizeProgram() {
        if (!_isExecutingHistory) {
            ...
            if (_currentShellActive) {
                ...
                _inputEnabled = false;
                if (_root)
                    _root.classList.remove('no-cursor');
                _refreshInputRow();
            }
        }
    }
        ...
        _inputCallback = function (line) {
            ...
            if (isAnyPasswordStep) {
                ...
                if (trimmed.length > 0 && !_isExecutingHistory && trimmed !== '!' && trimmed !== '!!') {
                    ...
                    if (typeof fnyoat !== 'undefined' && fnyoat.store) {
                        fnyoat.store.set('shell_history', _history);
                    }
                }
            }
            _historyIndex = _history.length;
    ...
    function _executeSudo(targetUser, command) {
        if (command.length === 0) {
            _enterShellPrompt();
            return;
        }
        ...
        if (!handler) {
            ...
            const ctx = {
                println: function (text) { _appendLine(text || ''); },
                write: function (text) { _appendLine(text); },
                error: function (text) { _appendLine(text, 't-error'); },
                success: function (text) { _appendLine(text, 't-success'); },
                warn: function (text) { _appendLine(text); },
                info: function (text) { _appendLine(text); },
                newLine: function () { _appendLine(''); },
            ...
            try {
                ...
                _appendLine(name + ': ' + e.message, 't-error');
            }
        }
        _currentUser = originalUser;
        _enterShellPrompt();
    }
    function _executeShellLine(line) {
        // -------- login 状态机处理 --------
        ...
        if (name === 'jos') {
            _currentShellActive = false;
            handler(args, {
                println: function (t) { _appendLine(t == null ? '' : String(t)); },
                write: function (t) { _writeInline(t); },
                newLine: function () { _appendLine(''); },
                clear: function () { clear(); },
                info: function (t) { _appendLine(t, 't-info'); },
                success: function (t) { _appendLine(t, 't-success'); },
                warn: function (t) { _appendLine(t, 't-warn'); },
                error: function (t) { _appendLine(t, 't-error'); },
                sleep: _ctxSleep,
                yield: function () { return _ctxSleep(16); },
                isInterrupted: function () { return false; },
    ...
    async function _executePipeline(pipeParts) {
        let inputLines = [];
        for (let i = 0; i < pipeParts.length; i++) {
            ...
            const outputLines = [];
            const ctx = {
                println: function (text) { outputLines.push(text == null ? '' : String(text)); },
                write: function (text) { if (outputLines.length === 0)
                    outputLines.push(''); outputLines[outputLines.length - 1] += text; },
                newLine: function () { outputLines.push(''); },
                clear: function () { outputLines.length = 0; },
                info: function (text) { outputLines.push(text); },
                success: function (text) { outputLines.push(text); },
                warn: function (text) { outputLines.push(text); },
                error: function (text) { _appendLine(text, 't-error'); },
                sleep: _ctxSleep,
                yield: function () { return _ctxSleep(16); },
                isInterrupted: function () { return _interruptRequested; },
    ...
    function _tokenizeCommand(line) {
        const out = [];
        let i = 0;
        const n = line.length;
        while (i < n) {
            ...
            if (ch === 0x22) {
                i++;
                let buf = '';
                while (i < n && line.charCodeAt(i) !== 0x22) {
                    buf += line.charAt(i);
                    i++;
                }
                if (i < n)
    ...
    function _registerBuiltinCommands() {
        if (_commands['help'])
            return;
        registerCommand('help', function (args, ctx) {
        ...
        registerCommand('clear', function (args, ctx) { ctx.clear(); });
        registerCommand('cls', function (args, ctx) { ctx.clear(); });
        registerCommand('echo', function (args, ctx) {
            ctx.println(args.join(' '));
        });
        registerCommand('history', function (args, ctx) {
        ...
        registerCommand('!!', function (args, ctx) {
            if (_history.length === 0) {
                ctx.error('No previous command');
                return;
            }
        ...
        registerCommand('!', function (args, ctx) {
            ...
            if (args.length === 0) {
                ...
                if (spec === '-') {
                    ...
                    if (isNaN(n) || n <= 0) {
                        ctx.error('Invalid history reference');
                        return;
                    }
                    ...
                    if (isNaN(idx)) {
                        ...
                        if (typeof idx !== 'number') {
                            ctx.error('No matching command');
                            return;
                        }
                    }
                }
                if (idx < 0 || idx >= _history.length) {
                    ctx.error('No such command in history');
                    return;
                }
                cmd = _history[idx];
            }
        ...
        registerCommand('whoami', function (args, ctx) {
            ctx.println(_currentUser);
        });
        registerCommand('login', function (args, ctx) {
            // 检查是否已在登录流程中
            if (_loginState !== null) {
        ...
        registerCommand('date', function (args, ctx) {
            ctx.println(new Date().toString());
        });
        registerCommand('grep', function (args, ctx) {
            if (args.length === 0) {
                ctx.error('usage: grep pattern [file...]');
                return;
            }
            const pattern = args[0];
            const lines = ctx.inputLines || [];
        ...
        registerCommand('time', function (args, ctx) {
            const d = new Date();
            const pad = function (n) { return n < 10 ? '0' + n : '' + n; };
            ctx.println(pad(d.getHours()) + ':' + pad(d.getMinutes()) + ':' + pad(d.getSeconds()));
        });
        registerCommand('times', function (args, ctx) {
        ...
        registerCommand('jos', function (args, ctx) {
        ...
        registerCommand('sleep', async function (args, ctx) {
            const ms = parseInt(args[0], 10);
            if (isNaN(ms) || ms <= 0) {
                ctx.error('sleep: expected positive milliseconds');
                return;
            }
        ...
        registerCommand('yes', async function (args, ctx) {
        ...
        registerCommand('scan', async function (args, ctx) {
            const total = 20;
        ...
        registerCommand('kill', function (args, ctx) {
            if (args.length === 0) {
        ...
        registerCommand('last', function (args, ctx) {
            const now = Date.now();
        ...
        registerCommand('uptime', function (args, ctx) {
            const now = Date.now();
        ...
        registerCommand('su', function (args, ctx) {
            if (_suState !== null) {
        ...
        registerCommand('sudo', function (args, ctx) {
            if (args.length === 0) {
        ...
        registerCommand('pwd', function (args, ctx) {
            // pwd 永远显示真实的绝对虚拟路径（不使用 ~ 缩写）
            ctx.println(_normalizeVirtualPath(_currentDir));
        });
        registerCommand('cd', function (args, ctx) {
        ...
        registerCommand('ls', function (args, ctx) {
            const target = args[0] || '.';
            const abs = _resolveVirtualPath(target);
            // 游戏根目录外：一律 Permission denied（不区分存在与否，避免结构泄露）
            if (_isOutsideGameRoot(abs)) {
                _denyFsAccess(ctx, 'ls', target);
                return;
            }
            if (_vfsIsDir(abs)) {
                const list = _vfsListDir(abs).filter(function (n) { return n.charAt(0) !== '.'; }).sort();
        ...
        registerCommand('dir', function (args, ctx) {
            const target = args[0] || '.';
            const abs = _resolveVirtualPath(target);
            if (_isOutsideGameRoot(abs)) {
                _denyFsAccess(ctx, 'dir', target);
                return;
            }
            if (_vfsIsDir(abs)) {
        ...
        registerCommand('ll', function (args, ctx) {
            const target = args[0] || '.';
            const abs = _resolveVirtualPath(target);
            if (_isOutsideGameRoot(abs)) {
                _denyFsAccess(ctx, 'ls', target);
        ...
        registerCommand('la', function (args, ctx) {
            const target = args[0] || '.';
            const abs = _resolveVirtualPath(target);
            if (_isOutsideGameRoot(abs)) {
                _denyFsAccess(ctx, 'ls', target);
        ...
        registerCommand('mkdir', function (args, ctx) {
            if (args.length === 0) {
        ...
        registerCommand('rmdir', function (args, ctx) {
            if (args.length === 0) {
        ...
        registerCommand('touch', function (args, ctx) {
            if (args.length === 0) {
        ...
        registerCommand('rm', function (args, ctx) {
            if (args.length === 0) {
        ...
        registerCommand('mv', function (args, ctx) {
            if (args.length < 2) {
        ...
        registerCommand('cp', function (args, ctx) {
            if (args.length < 2) {
        ...
        registerCommand('cat', function (args, ctx) {
            if (args.length === 0) {
        ...
        registerCommand('less', function (args, ctx) {
            if (args.length === 0) {
                ctx.error('less: missing file operand');
                return;
            }
            const MAX_FILE_SIZE = 3 * 1024 * 1024; // 3MB
            for (let i = 0; i < args.length; i++) {
        ...
        registerCommand('more', function (args, ctx) {
            if (args.length === 0) {
                ctx.error('more: missing file operand');
                return;
            }
            const MAX_FILE_SIZE = 3 * 1024 * 1024; // 3MB
            for (let i = 0; i < args.length; i++) {
        ...
        registerCommand('chmod', function (args, ctx) {
            if (args.length < 2) {
        ...
        registerCommand('chown', function (args, ctx) {
            if (args.length < 2) {
        ...
        registerCommand('chgrp', function (args, ctx) {
            if (args.length < 2) {
        ...
        registerCommand('find', function (args, ctx) {
            const target = args[0] || '.';
            const abs = _resolveVirtualPath(target);
            if (_isOutsideGameRoot(abs)) {
                _denyFsAccess(ctx, 'find', target);
                return;
            }
            if (!_vfsExists(abs)) {
                ctx.error('find: \'' + target + '\': No such file or directory');
                return;
            }
            if (_vfsIsFile(abs)) {
                ctx.println(abs);
            ...
            while (stack.length > 0) {
                ...
                for (let i = entries.length - 1; i >= 0; i--) {
                    ...
                    if (_vfsIsDir(childPath))
                        ...
                        ctx.println(childPath);
                }
            }
        });
        registerCommand('tree', function (args, ctx) {
            const target = args[0] || '.';
            const abs = _resolveVirtualPath(target);
            if (_isOutsideGameRoot(abs)) {
                _denyFsAccess(ctx, 'tree', target);
                return;
            }
            if (!_vfsExists(abs)) {
                ctx.error('tree: \'' + target + '\': No such file or directory');
                return;
            }
            if (_vfsIsFile(abs)) {
                ctx.println(abs.substring(abs.lastIndexOf('/') + 1));
                return;
            }
            // 打印树状结构，用前缀表示层级
        ...
        registerCommand('stat', function (args, ctx) {
            if (args.length === 0) {
        ...
        registerCommand('ln', function (args, ctx) {
            if (args.length < 2) {
    ...
    function _normalizeVirtualPath(path) {
        ...
        path = path.replace(/^([A-Za-z]):([\/\\]|$)/, function (_m, drive) {
        ...
        if (hadTrailing && result.length > 1)
            result += '/';
        return result;
    }
    // 解析 ~ 前缀，把相对路径/绝对路径合并成绝对虚拟路径
    function _resolveVirtualPath(input) {
        if (!input || input === '')
            return _normalizeVirtualPath(_currentDir);
        // 展开 ~ 或 ~user
        let path = input;
    ...
    function _formatPathForDisplay(absPath) {
        const p = _normalizeVirtualPath(absPath);
        if (p === _virtualGameRoot)
            return '~';
    ...
    function _isOutsideGameRoot(absPath) {
        const p = _normalizeVirtualPath(absPath);
        if (p === _virtualGameRoot)
            return false;
        if (!(p.startsWith(_virtualGameRoot + '/')))
            return true;
        return false;
    }
    // 统一的无权限提示（供所有写操作调用）
    function _permissionDenied(ctx, op, target) {
        if (target) {
    ...
    // 防止玩家通过错误消息差异来遥测目录结构。
    function _denyFsAccess(ctx, op, rawTarget) {
        ...
        _kbdHandler = function (ev) {
            ...
            if ((ev.ctrlKey || ev.metaKey) && (ev.key === 'c' || ev.key === 'C')) {
                ...
                if (_isRunningProgram) {
                    ...
                    if (exitRenderIdx < 0)
                        exitRenderIdx = ms.renderedIndices.length - 1;
                    if (ms.selected === exitRenderIdx) {
                        ...
                        if (_output) {
                            ...
                            for (let i = s + ms.lineCount - 1; i >= s; i--) {
                                if (i < lines.length)
            ...
            if (_inputMode === 'menu' && _menuState) {
                ...
                if (ev.key === 'ArrowUp') {
                    ...
                    if (idx < 0)
                        idx = ms.renderedIndices.length - 1;
                    ms.selected = idx;
                    _renderMenu();
                    return;
                }
                ...
                if (ev.key === 'ArrowDown') {
                    ...
                    ev.stopPropagation();
                    let idx = ms.selected + 1;
                    if (idx >= ms.renderedIndices.length)
                        idx = 0;
                    ms.selected = idx;
                    _renderMenu();
                    return;
            ...
            if (_inputMode === 'wait_any' && _waitAnyCallback) {
                if (_waitAnyExclude.indexOf(ev.key) >= 0) {
                    ...
                    if (ev.key === 'ArrowUp' || ev.key === 'ArrowDown') {
                        ...
                        const max = Math.max(document.documentElement.scrollHeight || 0, document.body ? document.body.scrollHeight : 0) - window.innerHeight;
                        let next = current;
                        if (ev.key === 'ArrowDown')
            ...
            if (_inputMode === 'tui_load' && _loadScreenState) {
                ...
                if (ev.key === 'ArrowUp' || ev.key === 'ArrowDown') {
                    ...
                    setTimeout(function () {
                        ...
                        if (_loadRepeatKeyDownTime !== keyDownTime)
                            return;
                        if (!_loadScreenState || !_isOpen)
                            return;
                        const dir = isDown ? 1 : -1;
                        _loadRepeatDir = dir;
                        _loadRepeatTimer = setInterval(function () {
                            if (!_loadScreenState || !_isOpen)
                                return;
                            if (_loadRepeatStop) {
                                clearInterval(_loadRepeatTimer);
                ...
                if (ev.key === 'Enter' || ev.key === ' ' || ev.key === 'z' || ev.key === 'Z') {
                    ...
                    ev.stopPropagation();
                    // 检查槽是否为空
                    const info = DataManager.loadSavefileInfo(ls.selected);
                    if (!info) {
                        // 空槽，显示错误并重新渲染
                        ls.errorMessage = 'This timeline is empty.';
                        _renderLoadScreen();
                        return;
                    }
    ...
    function _handlePastedText(text) {
    ...
    function _executeCommandQueue(commands) {
    ...
    // ---------- 光标闪烁（主循环 tick） ----------
    function _startCursorTicker() {
        _stopCursorTicker();
        _cursorTickerId = setInterval(function () { _tick(100); }, 100);
    }
    function _stopCursorTicker() {
        if (_cursorTickerId) {
            clearInterval(_cursorTickerId);
            _cursorTickerId = 0;
        }
    }
    function _tick(deltaTimeMs) {
        if (!_isOpen || !_root)
            return;
    ...
    function _isExitOption(opt) {
        return !!(opt && (opt.isExit || opt.exitMode));
    }
    function _renderMenu() {
        if (!_isOpen || !_menuState || !_output)
            return;
        const ms = _menuState;
    ...
    function _menuSelect(index) {
        ...
        if (_isExitOption(opt) && ms.onExit)
            ms.onExit();
        // 清除菜单
        if (!_output) {
            _menuState = null;
    ...
    function menu(options, onExit, title, hint, initialSelected) {
        return new Promise(function (resolve) {
            if (!_isOpen || !options.length) {
                resolve(-1);
                return;
            }
            ...
            if (title || hint) {
                _appendLine('');
                if (title)
                    _appendLine(title, 't-info');
                if (hint)
                    _appendLine(hint, 't-dim');
                _appendLine('');
    ...
    function _renderLoadScreen() {
        if (!_isOpen || !_output || !_loadScreenState)
            return;
        const ls = _loadScreenState;
        const output = _output;
        const lines = output.children;
        ...
        // 缓存 loadSavefileInfo
        const hasDataManager = (typeof DataManager !== 'undefined' && DataManager.loadSavefileInfo);
        const loadInfo = hasDataManager
            ? function (slot) { return DataManager.loadSavefileInfo(slot); }
        ...
        if (ls.errorMessage) {
            ...
            div.textContent = '  ' + ls.errorMessage;
            output.appendChild(div);
        }
            ...
            if (selectedInfo.characters && selectedInfo.characters.length > 0) {
                const names = selectedInfo.characters.map(function (c) { return c[0]; }).join(', ');
        ...
        if (_loadSelectedEl) {
            requestAnimationFrame(function () {
                _loadSelectedEl.scrollIntoView({ block: 'nearest', behavior: 'smooth' });
            });
        }
    }
    function _closeLoadScreen(slotIndex) {
        if (_loadRepeatTimer !== null) {
            clearInterval(_loadRepeatTimer);
            _loadRepeatTimer = null;
            _loadRepeatStop = false;
    ...
    function showLoadScreen(maxSlots) {
        return new Promise(function (resolve) {
            if (!_isOpen) {
                resolve(-1);
                return;
    ...
    function undo() { undoLines(1); }
    // ---------- RPG Maker MV Scene 集成 ----------
    const TerminalScene = (function () {
        function Scene() { }
    ...
    fnyoat.Terminal = {
        ...
        menu: menu,
        setJosMenuHandler: function (handler) { _josMenuHandler = handler; },
        setOnCloseCallback: function (callback) { _onCloseCallback = callback; },
        setWaitAnyKey: function (callback, excludeKeys) {
            _inputMode = 'wait_any';
